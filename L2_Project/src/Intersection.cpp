...
void WaitingVehicles::permitEntryToFirstInQueue()
{
    ...
    auto firstVehiclePrms = _promises.front();
    auto firstVehicle = _vehicles.front();

    // Fulfill promise and send signal back that permission to enter has been granted
    firstVehiclePrms.set_value();

    // Remove the front elements from both queues
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    std::promise<void> prmsVehiclesAllowedToEnter;
    std::future<void> ftrVehiclesAllowedToEnter = prmsVehiclesAllowedToEnter.get_future();

    _waitingVehicles.pushBack(vehicle, std::move(prmsVehiclesAllowedToEnter));

    // Wait until the vehicle has been granted entry
    ftrVehiclesAllowedToEnter.wait();
